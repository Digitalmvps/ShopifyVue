...
export default function nodejs(appName) {
  ...
  if (!shell.which("git")) {
    ...
    shell.exec(
      `git clone https://github.com/Doctordrayfocus/vue-typescript-template.git ${appName}`
    );
    ...
    // update index.html
    copyFileOrFolder("/bin/nodejs/index.html", "public/index.html");
    ...
    // remove .git file
    shell.exec(`cd ${fullAppDir} && rm -rf .git`);
